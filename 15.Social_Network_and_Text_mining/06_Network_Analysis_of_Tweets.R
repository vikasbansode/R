...
#tdm <- tdm[rowSums(tdm) > 30,] # reducing numbers when document is too big
...
# Set labels of vertices to tweet ID's
V(g)$label <- V(g)$name
V(g)$label.cex <- 1
...
E(g)$color <- rgb(.5,.5,0,egam)
E(g)$width <- egam
g2 <- delete.vertices(g,V(g)[degree(g) < 5])
...
# Checking isolated tweets from the group
